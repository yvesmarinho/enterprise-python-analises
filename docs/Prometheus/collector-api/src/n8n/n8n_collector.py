...
class N8NCollector:
    ...
    async def run_periodic_collection(self, interval: int = 60) -> None:
        ...
        # Health check inicial com retry — não sai da task se N8N estiver temporariamente indisponível
        startup_delay = interval
        while True:
            is_healthy = await self.client.health_check()
            if is_healthy:
                break
            logger.warning("n8n_api_not_available_at_startup_retrying",
                          retry_in_seconds=startup_delay)
            await asyncio.sleep(startup_delay)
            startup_delay = min(startup_delay * 2, interval * 10)
        ...
        while True:
            try:
                ...
                logger.info("n8n_periodic_collection_cancelled")
                break
            except Exception as e:
