...
class PostgresProbe:
    """Probe para monitorar PostgreSQL"""
    ...
    def __init__(self):
        ...
        logger.info("postgres_probe_initialized", host=settings.postgres_host)
    ...
    async def check_connection(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            async with await psycopg.AsyncConnection.connect(
                ...
                logger.debug("postgres_health_check_success",
            ...
            logger.error("postgres_unknown_error",
                        error=error_msg,
    ...
    async def execute_test_query(self) -> Tuple[bool, Optional[float]]:
        ...
        try:
            async with await psycopg.AsyncConnection.connect(
                ...
                logger.debug("postgres_test_query_success",
            ...
            logger.error("postgres_test_query_error",
                        error=str(e),
                        latency_ms=round(elapsed * 1000, 2))
    ...
    async def run_periodic_probe(self, interval: int = 60):
        ...
        """
        logger.info("postgres_periodic_probe_started", interval_seconds=interval)
        ...
        while True:
            try:
                ...
                logger.info("postgres_periodic_probe_cancelled")
                break
            except Exception as e:
                logger.error("postgres_periodic_probe_error", error=str(e))
                await asyncio.sleep(interval)
