...
```bash
sudo mkdir -p /opt/docker_user/n8n-monitoring-local/{victoria-data,grafana-data,grafana-provisioning,logs/collector-api}
sudo chown -R $USER:$USER /opt/docker_user/n8n-monitoring-local
```

### 3. Iniciar Serviços

```bash
cd /opt/docker_user/n8n-monitoring-local/deploy/wf001-usa
docker-compose up -d
```
